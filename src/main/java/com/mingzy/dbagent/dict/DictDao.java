...
public class DictDao {
    ...
    private static final RowMapper<DictItem> MAPPER = (rs, i) -> new DictItem(
            ...
            rs.getInt("sort"), rs.getInt("enabled") == 1, rs.getString("ext_value"));

    public long insert(DictItem d) {
        KeyHolder kh = new GeneratedKeyHolder();
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement(
                "INSERT INTO sys_dict(dict_type, dict_key, dict_label, parent_key, sort, enabled, ext_value) VALUES(?,?,?,?,?,?,?)",
    ...
    public void update(DictItem d) {
        jdbc.update("UPDATE sys_dict SET dict_type=?, dict_key=?, dict_label=?, parent_key=?, sort=?, enabled=?, ext_value=? WHERE id=?",
                d.dictType(), d.dictKey(), d.dictLabel(), d.parentKey(), d.sort(), d.enabled() ? 1 : 0, d.extValue(), d.id());
    }
